  ...
  private readonly logger = new Logger('GlobalExceptionFilter');
    ...
    if (exception instanceof HttpException) {
      ...
      switch (exception.code) {
        ...
        case 'P2003': {
          ...
          message = 'Erreur de clé étrangère (une relation requise est manquante ou invalide).';
          break;
        }
        default: {
    ...
    const responseBody = typeof message === 'string'
      ...
      : {
          ...
          timestamp: new Date().toISOString(),
          path: request.url,
          ...(message as object), // If it's the default Nest validation error object (which includes validation messages)
        };
